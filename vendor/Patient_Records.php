...
<head>
  ...
  <style>
    ...
    .sidenav a:hover {
      background-color:rgb(154, 81, 81);;
      color:white;
      border-radius:100px;
    }
      ...
      <?php
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<button style='background-color:rgb(69, 13, 13); color:white;margin-bottom:10px; ' onclick='popUp(\"{$row['RecordID']}\", \"{$row['PatientName']}\", \"{$row['DoctorName']}\", \"{$row['Diagnosis']}\", \"{$row['Prescription']}\", \"{$row['Date']}\")'>Update</button>";
